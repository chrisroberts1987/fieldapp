...
async function cascade({ invoiceId, orgId, userId, customerId, amount, paidVia }) {
  // Fetch customer (for name + email)
  let cust = null;
  if (customerId) {
    const { data } = await supabase.from('customers').select('id, name, email').eq('id', customerId).maybeSingle();
    cust = data;
  }
  ...
  // Customer emails: branded receipt FIRST (transactional acknowledgement),
  // then the feedback request. Two separate sends so a customer reading
  // their inbox sees the receipt as the immediate confirmation, with the
  // review ask as a follow-up — not tangled together.
  if (cust?.email && typeof window !== 'undefined') {
    ...
    await sendEmail({
      ...
      to: cust.email,
      data: { customerName: custName, invoiceNumber, amount, paidVia, paidDate, invoiceUrl },
    });
  }
  if (cust?.email && existing?.token && typeof window !== 'undefined') {
