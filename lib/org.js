  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: o } = await supabase
        .from('organizations')
        .select('id, name, slug, plan, trial_ends_at, owner_name, phone, business_email, address, logo_url, license_number, default_tax_rate, income_tax_rate, stripe_customer_id, stripe_subscription_id, subscription_status, subscription_tier, subscription_current_period_end, subscription_cancel_at_period_end')
        .eq('id', m.org_id)
        .maybeSingle();
