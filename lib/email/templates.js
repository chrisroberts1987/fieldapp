...
const FACEBOOK_URL = 'https://www.facebook.com/profile.php?id=61590402943484';
...
export function welcomeEmail({ ownerName, businessName }) {
  ...
  const body = `
    ...
    <p style="margin:0 0 14px;">One more thing. We post product updates, tips from other contractors, and behind-the-scenes builds on our Facebook page. Follow along to keep up with what's coming and connect with other contractors using MyForeman.</p>
    <p style="margin:0 0 18px;">${ctaButton(FACEBOOK_URL, 'Follow MyForeman on Facebook')}</p>
...
// ============================================================
// Change order — added work after the original quote was approved.
// Same approval flow as a quote: customer taps Review & Approve,
// hits a public page, then approves or declines.
// ============================================================
export function changeOrderRequestEmail({ org, customerName, jobTitle, description, reason, amount, approvalUrl }) {
  const name = customerName || 'there';
  const job  = jobTitle ? ` on "${escapeHtml(jobTitle)}"` : '';
  const why  = reason ? `<p style="margin:0 0 14px;color:#4a5468;font-size:14px;"><strong style="color:#1a1f2b;">Why:</strong> ${escapeHtml(reason)}</p>` : '';
  const what = description ? `<p style="margin:0 0 14px;color:#4a5468;font-size:14px;"><strong style="color:#1a1f2b;">Additional work:</strong> ${escapeHtml(description)}</p>` : '';
  const body = `
    <p style="margin:0 0 14px;">Hi ${escapeHtml(name)},</p>
    <p style="margin:0 0 14px;">Quick heads up: we found some additional work needed${job}. Before we move forward, we need your sign-off on the change.</p>
    <div style="margin:18px 0;padding:14px 16px;background:#f5f7fb;border-radius:8px;">
      <div style="font-size:12px;color:#7a8395;letter-spacing:.06em;text-transform:uppercase;font-weight:600;">Change order</div>
      <div style="font-size:28px;font-weight:700;color:#1a1f2b;letter-spacing:.01em;margin-top:4px;">${fmtMoney(amount)}</div>
    </div>
    ${what}
    ${why}
    <p style="margin:0 0 16px;">Review the full breakdown and approve below. No account needed.</p>
    <p style="margin:0 0 18px;">${ctaButton(approvalUrl, 'Review & Approve')}</p>
    <p style="margin:18px 0 0;color:#4a5468;font-size:13px;">Reply to this email with any questions before approving.</p>
    <p style="margin:6px 0 0;color:#4a5468;font-size:13px;">— ${escapeHtml(org.name || 'Your contractor')}</p>
  `;
  return {
    subject: `Change order from ${org.name || 'us'} — ${fmtMoney(amount)}`,
    html: shell({ orgName: org.name, orgLogo: org.logo_url, bodyHtml: body }),
    text: `Hi ${name},\n\nQuick heads up: we found some additional work needed${jobTitle ? ` on "${jobTitle}"` : ''}.\n\nChange order: ${fmtMoney(amount)}${description ? `\nAdditional work: ${description}` : ''}${reason ? `\nWhy: ${reason}` : ''}\n\nReview & approve: ${approvalUrl}\n\nReply to this email with any questions.\n— ${org.name || 'Your contractor'}\n\n${FOOTER_TEXT}`,
  };
}
