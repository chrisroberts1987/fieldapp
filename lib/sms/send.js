...
// SMS is ready when we have account creds plus at least one sender —
// either a Messaging Service (preferred for A2P 10DLC) or a raw
// from-number (legacy / single-sender fallback).
...
export async function sendBrandedSMS({ to, body, statusCallback }) {
  ...
  // Prefer the Messaging Service SID when set — gives A2P 10DLC
  // routing, number-pool failover, and better deliverability than a
  // single from-number. Falls back to TWILIO_FROM_NUMBER for installs
  // that haven't created a Messaging Service yet.
